...
static void convertMergeTreeToReplicated(Poco::Logger * log, ContextMutablePtr context, const DatabasePtr & database, StorageID id)
{
    ...
    LOG_INFO(log, "Finished conversion from MergeTree to ReplicatedMergeTree on");
}
...
static void findAndConvertMergeTreeTablesToReplicated(ContextMutablePtr context, const String & database_name)
{
    ...
    {
        e.addMessage("Exception while trying to convert MergeTree tables in database {}. It may be in some intermediate state."
            " You can finish conversion manually by moving the rest tables from {} to {} (using RENAME TABLE)"
            " and executing DROP DATABASE {} and RENAME DATABASE {} TO {}.",
